...
// #include "hwacha_objc_platform_dependent.h"
...
namespace Halide { namespace Runtime { namespace Internal { namespace Hwacha {

// typedef halide_hwacha_device mtl_device;
// typedef halide_hwacha_command_queue mtl_command_queue;
// struct mtl_buffer;
...
extern WEAK halide_device_interface_t hwacha_device_interface;
...
// WEAK bool hwacha_api_supports_set_bytes;
// WEAK mtl_device *hwacha_api_checked_device;
...
using namespace Halide::Runtime::Internal::Hwacha;

extern "C" {

// The default implementation of halide_hwacha_acquire_context uses the global
// pointers above, and serializes access with a spin lock.
// Overriding implementations of acquire/release must implement the following
// behavior:
// - halide_acquire_hwacha_context should always store a valid device/command
//   queue in device/q, or return an error code.
// - A call to halide_acquire_hwacha_context is followed by a matching call to
//   halide_release_hwacha_context. halide_acquire_hwacha_context should block while a
//   previous call (if any) has not yet been released via halide_release_hwacha_context.
// WEAK int halide_hwacha_acquire_context(void *user_context, mtl_device **device_ret,
...
//     if (device == 0 && create) {
//         debug(user_context) <<  "Hwacha - Allocating: MTLCreateSystemDefaultDevice\n";
//         device = (mtl_device *)MTLCreateSystemDefaultDevice();
//         if (device == 0) {
//             error(user_context) << "Hwacha: cannot allocate system default device.\n";
//             __sync_lock_release(&thread_lock);
//             return -1;
//         }
//         debug(user_context) <<  "Hwacha - Allocating: new_command_queue\n";
//         queue = new_command_queue(device);
//         if (queue == 0) {
//             error(user_context) << "Hwacha: cannot allocate command queue.\n";
...
// WEAK int halide_hwacha_release_context(void *user_context) {
//     __sync_lock_release(&thread_lock);
//     return 0;
// }
...
//     __attribute__((always_inline)) HwachaContextHolder(void *user_context, bool create) { save(user_context, create); }
//     __attribute__((always_inline)) ~HwachaContextHolder() { restore(); }
// };

// WEAK void HwachaContextHolder::save(void *user_context_arg, bool create) {
//     user_context = user_context_arg;
//     pool = create_autorelease_pool();
//     error = halide_hwacha_acquire_context(user_context, &device, &queue, create);
// }

// WEAK void HwachaContextHolder::restore() {
//     halide_hwacha_release_context(user_context);
...
}}}} // namespace Halide::Runtime::Internal::hwacha
...
WEAK int halide_hwacha_device_malloc(void *user_context, halide_buffer_t* buf) {
  debug(user_context) << "hwacha_malloc\n";
    debug(user_context)
        << "halide_hwacha_device_malloc (user_context: " << user_context
        << ", buf: " << buf << ")\n";
    ...
    // HwachaContextHolder hwacha_context(user_context, true);
    // if (hwacha_context.error != 0) {
    //     return hwacha_context.error;
    // }
    ...
    // mtl_buffer *hwacha_buf = new_buffer(hwacha_context.device, size);
    // if (hwacha_buf == 0) {
    //     free(handle);
    //     error(user_context) << "Hwacha: Failed to allocate buffer of size " << (int64_t)size << ".\n";
    //     return -1;
    // }

    // handle->buf = hwacha_buf;
    // handle->offset = 0;

    // buf->device = (uint64_t)handle;
    buf->device = 1;
    buf->device_interface = &hwacha_device_interface;
...
WEAK int halide_hwacha_device_free(void *user_context, halide_buffer_t* buf) {
    debug(user_context) << "hwacha_free\n";
    debug(user_context) << "halide_hwacha_device_free called on buf "
    ...
    // halide_assert(user_context, (((device_handle *)buf->device)->offset == 0) && "halide_hwacha_device_free on buffer obtained from halide_device_crop");
...
WEAK int halide_hwacha_initialize_kernels(void *user_context, void **state_ptr, const char* source, int source_size) {
  debug(user_context) << "hwacha_init_kernel\n";
  return 1;
    ...
    //     debug(user_context) << "Hwacha - Allocating: new_library_with_source " << (*state)->library << "\n";
    //     (*state)->library = new_library_with_source(hwacha_context.device, source, source_size);
    //     if ((*state)->library == 0) {
    //         error(user_context) << "Hwacha: new_library_with_source failed.\n";
    //         return -1;
    //     }

    //     #ifdef DEBUG_RUNTIME
    //     uint64_t t_after_compile = halide_current_time_ns(user_context);
    //     debug(user_context) << "Time for halide_hwacha_initialize_kernels compilation: " << (t_after_compile - t_before_compile) / 1.0e6 << " ms\n";
    //     #endif
    // }

    // #ifdef DEBUG_RUNTIME
    // uint64_t t_after = halide_current_time_ns(user_context);
    // debug(user_context) << "Time for halide_hwacha_initialize_kernels: " << (t_after - t_before) / 1.0e6 << " ms\n";
    // #endif
...
// inline void halide_hwacha_device_sync_internal(mtl_command_queue *queue, struct halide_buffer_t *buffer) {
//     mtl_command_buffer *sync_command_buffer = new_command_buffer(queue);
//     if (buffer != NULL) {
//         mtl_buffer *hwacha_buffer = ((device_handle *)buffer->device)->buf;
//         if (is_buffer_managed(hwacha_buffer)) {
//             mtl_blit_command_encoder *blit_encoder = new_blit_command_encoder(sync_command_buffer);
//             synchronize_resource(blit_encoder, hwacha_buffer);
...
WEAK int halide_hwacha_device_sync(void *user_context, struct halide_buffer_t *buffer) {
  debug(user_context) << "hwacha_sync\n";
    // #ifdef DEBUG_RUNTIME
    // uint64_t t_before = halide_current_time_ns(user_context);
    // #endif
    ...
    // halide_hwacha_device_sync_internal(hwacha_context.queue, buffer);

    // #ifdef DEBUG_RUNTIME
    // uint64_t t_after = halide_current_time_ns(user_context);
    // debug(user_context) << "Time for halide_hwacha_device_sync: " << (t_after - t_before) / 1.0e6 << " ms\n";
    // #endif

    return 0;
}

WEAK int halide_hwacha_device_release(void *user_context) {
  debug(user_context) << "hwacha_release\n";
    // // The HwachaContext object does not allow the context storage to be modified,
    // // so we use halide_hwacha_acquire_context directly.
    // int error;
    // mtl_device *acquired_device;
    // mtl_command_queue *acquired_queue;
    // error = halide_hwacha_acquire_context(user_context, &acquired_device, &acquired_queue, false);
    // if (error != 0) {
    //     return error;
    // }

    // if (device) {
    //     halide_hwacha_device_sync_internal(queue, NULL);
    ...
    //     while (state) {
    //       if (state->library) {
    //             debug(user_context) << "Hwacha - Releasing: new_library_with_source " << state->library << "\n";
    ...
    //         debug(user_context) <<  "Hwacha - Releasing: new_command_queue " << queue << "\n";
    //         release_ns_object(queue);
    //         queue = NULL;

    //         debug(user_context) << "Hwacha - Releasing: MTLCreateSystemDefaultDevice " << device << "\n";
    //         release_ns_object(device);
    //         device = NULL;
    //     }
    // }
...
WEAK int halide_hwacha_copy_to_device(void *user_context, halide_buffer_t* buffer) {
  debug(user_context) << "hwacha_copy_to_device\n";
  debug(user_context) << *buffer << "\n";
    // #ifdef DEBUG_RUNTIME
    // uint64_t t_before = halide_current_time_ns(user_context);
    // #endif
    ...
    // mtl_buffer *hwacha_buffer = ((device_handle *)c.dst)->buf;
    // c.dst = (uint64_t)buffer_contents(hwacha_buffer) + ((device_handle *)c.dst)->offset;

    // debug(user_context) << "halide_hwacha_copy_to_device dev = " << (void*)buffer->device
    //                     << " hwacha_buffer = " << hwacha_buffer
    //                     << " host = " << buffer->host << "\n";
    ...
    //     did_modify_range(hwacha_buffer, total_extent);
    // }
    // halide_hwacha_device_sync_internal(hwacha_context.queue, buffer);

    // #ifdef DEBUG_RUNTIME
    // uint64_t t_after = halide_current_time_ns(user_context);
    // debug(user_context) << "Time for halide_hwacha_copy_to_device: "
...
WEAK int halide_hwacha_copy_to_host(void *user_context, halide_buffer_t* buffer) {
  debug(user_context) << "hwacha_copy_to_host " << *buffer << "\n";
    // #ifdef DEBUG_RUNTIME
    // uint64_t t_before = halide_current_time_ns(user_context);
    // #endif
    ...
    // #ifdef DEBUG_RUNTIME
    // uint64_t t_after = halide_current_time_ns(user_context);
    // debug(user_context) << "Time for halide_hwacha_copy_to_host: " << (t_after - t_before) / 1.0e6 << " ms\n";
    // #endif

    return 0;
}

WEAK int halide_hwacha_run(void *user_context,
                           void *state_ptr,
  ...
  debug(user_context) << "hwacha_run\n";
    ...
    // mtl_command_buffer *command_buffer = new_command_buffer(hwacha_context.queue);
    // if (command_buffer == 0) {
    //     error(user_context) << "Hwacha: Could not allocate command buffer.\n";
    //     return -1;
    // }

    // mtl_compute_command_encoder *encoder = new_compute_command_encoder(command_buffer);
    // if (encoder == 0) {
    //     error(user_context) << "Hwacha: Could not allocate compute command encoder.\n";
    //     return -1;
    // }
    ...
    // if (function == 0) {
    //     error(user_context) << "Hwacha: Could not get function " << entry_name << "from Hwacha library.\n";
    //     return -1;
    // }

    // mtl_compute_pipeline_state *pipeline_state = new_compute_pipeline_state_with_function(hwacha_context.device, function);
    // if (pipeline_state == 0) {
    //     error(user_context) << "Hwacha: Could not allocate pipeline state.\n";
    //     return -1;
    // }
    ...
    //     if (!arg_is_buffer[i]) {
    //         // Hwacha requires natural alignment for all types in structures.
    ...
    //     if (hwacha_api_checked_device != hwacha_context.device) {
    //         hwacha_api_supports_set_bytes = buffer_supports_set_bytes(encoder);
    //         hwacha_api_checked_device = hwacha_context.device;
    //         if (hwacha_api_supports_set_bytes) {
    //             debug(user_context) << "Hwacha - supports setBytes\n";
    //         }
    //     }

    //     // The Hwacha compiler introduces padding up to a multiple of 4 bytes
    ...
    //     if (padded_args_size < 4096 && hwacha_api_supports_set_bytes) {
    //         args_ptr = (char *)small_args_buffer;
    //     } else {
    //         args_buffer = new_buffer(hwacha_context.device, padded_args_size);
    //         if (args_buffer == 0) {
    //             error(user_context) << "Hwacha: Could not allocate arguments buffer.\n";
    ...
    //     if (total_args_size < 4096 && hwacha_api_supports_set_bytes) {
    ...
    // #ifdef DEBUG_RUNTIME
    // uint64_t t_after = halide_current_time_ns(user_context);
    // debug(user_context) << "Time for halide_hwacha_device_run: " << (t_after - t_before) / 1.0e6 << " ms\n";
    // #endif

    return 0;
}

WEAK int halide_hwacha_device_and_host_malloc(void *user_context, struct halide_buffer_t *buffer) {
  debug(user_context) << "hwacha_dev_and_host_malloc\n";
//     debug(user_context) << "halide_hwacha_device_and_host_malloc called.\n";
//     int result = halide_hwacha_device_malloc(user_context, buffer);
//     if (result == 0) {
//         mtl_buffer *hwacha_buffer = ((device_handle *)(buffer->device))->buf;
//         buffer->host = (uint8_t *)buffer_contents(hwacha_buffer);
//         debug(user_context) << "halide_hwacha_device_and_host_malloc"
...
WEAK int halide_hwacha_device_and_host_free(void *user_context, struct halide_buffer_t *buffer) {
  debug(user_context) << "hwacha_dev_and_host_free\n";
//     debug(user_context) << "halide_hwacha_device_and_host_free called.\n";
//     halide_hwacha_device_free(user_context, buffer);
//     buffer->host = NULL;
    return 0;
}

WEAK int halide_hwacha_buffer_copy(void *user_context, struct halide_buffer_t *src,
                                   const struct halide_device_interface_t *dst_device_interface,
                                   struct halide_buffer_t *dst) {
    debug(user_context) << "hwacha_buffer_copy\n";
//     if (dst->dimensions > MAX_COPY_DIMS) {
//         error(user_context) << "Buffer has too many dimensions to copy to/from GPU\n";
//         return halide_error_code_device_buffer_copy_failed;
//     }

//     // We only handle copies to hwacha buffers or to host
//     halide_assert(user_context, dst_device_interface == NULL ||
//                   dst_device_interface == &hwacha_device_interface);

//     if ((src->device_dirty() || src->host == NULL) &&
//         src->device_interface != &hwacha_device_interface) {
//         halide_assert(user_context, dst_device_interface == &hwacha_device_interface);
//         // This is handled at the higher level.
//         return halide_error_code_incompatible_device_interface;
//     }

//     bool from_host = (src->device_interface != &hwacha_device_interface) ||
//                      (src->device == 0) ||
...
//         debug(user_context)
//             << "halide_hwacha_buffer_copy (user_context: " << user_context
//             << ", src: " << src << ", dst: " << dst << ")\n";
...
//             debug(user_context) << "halide_hwacha_buffer_copy device to device case.\n";
//             mtl_command_buffer *blit_command_buffer = new_command_buffer(hwacha_context.queue);
...
//                 // are complete.
//                 halide_hwacha_device_sync_internal(hwacha_context.queue, src);
...
//                 // are complete.
//                 halide_hwacha_device_sync_internal(hwacha_context.queue, dst);
...
// WEAK int hwacha_device_crop_from_offset(void *user_context,
...
//     if (new_handle == NULL) {
//         error(user_context) << "halide_hwacha_device_crop: malloc failed making device handle.\n";
//         return halide_error_code_out_of_memory;
//     }
...
WEAK int halide_hwacha_device_crop(void *user_context,
                                    const struct halide_buffer_t *src,
                                    struct halide_buffer_t *dst) {
  debug(user_context) << "hwacha_copy\n";
//     const int64_t offset = calc_device_crop_byte_offset(src, dst);
//     return hwacha_device_crop_from_offset(user_context, src, offset, dst);
  return 0;
}

WEAK int halide_hwacha_device_slice(void *user_context,
                                    const struct halide_buffer_t *src,
                                    int slice_dim, int slice_pos,
                                    struct halide_buffer_t *dst) {
    debug(user_context) << "hwacha_slice\n";
//     const int64_t offset = calc_device_slice_byte_offset(src, slice_dim, slice_pos);
//     return hwacha_device_crop_from_offset(user_context, src, offset, dst);
  return 0;
}

WEAK int halide_hwacha_device_release_crop(void *user_context,
                                          struct halide_buffer_t *buf) {
  debug(user_context) << "hwacha_release_crop\n";
    // // Basically the same code as in halide_hwacha_device_free, but with
    // // enough differences to require separate code.

    // debug(user_context) << "halide_hwacha_device_release_crop called on buf "
    //                     << buf << " device is " << buf->device << "\n";
    // if (buf->device == 0) {
    //     return 0;
    // }
...
WEAK int halide_hwacha_wrap_buffer(void *user_context, struct halide_buffer_t *buf, uint64_t buffer) {
  debug(user_context) << "hwacha_wrap_buffer\n";
    ...
    // device_handle *handle = (device_handle *)malloc(sizeof(device_handle));
    // if (handle == NULL) {
    //     error(user_context) << "halide_hwacha_wrap_buffer: malloc failed making device handle.\n";
    //     return halide_error_code_out_of_memory;
    // }
...
WEAK int halide_hwacha_detach_buffer(void *user_context, struct halide_buffer_t *buf) {
  debug(user_context) << "hwacha_detach_buffer\n";
    // if (buf->device == 0) {
    //     return 0;
    // }
    // halide_assert(user_context, buf->device_interface == &hwacha_device_interface);
    // buf->device_interface->impl->release_module();
    // buf->device_interface = NULL;
...
// WEAK uintptr_t halide_hwacha_get_buffer(void *user_context, struct halide_buffer_t *buf) {
//     if (buf->device == NULL) {
//         return 0;
//     }
//     halide_assert(user_context, buf->device_interface == &hwacha_device_interface);
...
// WEAK uint64_t halide_hwacha_get_crop_offset(void *user_context, struct halide_buffer_t *buf) {
//     if (buf->device == NULL) {
//         return 0;
//     }
//     halide_assert(user_context, buf->device_interface == &hwacha_device_interface);
...
WEAK const struct halide_device_interface_t *halide_hwacha_device_interface() {
    return &hwacha_device_interface;
}

namespace {
__attribute__((destructor))
WEAK void halide_hwacha_cleanup() {
  //    halide_hwacha_device_release(NULL);
}
}
...
WEAK halide_device_interface_impl_t hwacha_device_interface_impl = {
    halide_use_jit_module,
    halide_release_jit_module,
    halide_hwacha_device_malloc,
    halide_hwacha_device_free,
    halide_hwacha_device_sync,
    halide_hwacha_device_release,
    halide_hwacha_copy_to_host,
    halide_hwacha_copy_to_device,
    halide_hwacha_device_and_host_malloc,
    halide_hwacha_device_and_host_free,
    halide_hwacha_buffer_copy,
    halide_hwacha_device_crop,
    halide_hwacha_device_slice,
    halide_hwacha_device_release_crop,
    halide_hwacha_wrap_buffer,
    halide_hwacha_detach_buffer
};

WEAK halide_device_interface_t hwacha_device_interface = {
